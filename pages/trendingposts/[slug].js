...
const PostDetails = ({ trending, trend }) => {
    ...
    <>
      ...
      <div className="items-center flex flex-col gap-2 p-2 ">
        ...
        <div className="flex flex-wrap gap-3">
          {trending.map((item) => (
            <TrendCard key={item._id} trend={item} />
          ))}
        </div>
      </div>
    </>
...
export const getStaticPaths = async () => {
  ...
  const paths = trending.map((trend) => ({
    params: {
      slug: trend.slug.current,
    },
  }));
...
export const getStaticProps = async ({ params: { slug } }) => {
  ...
  const trend = await client.fetch(query);
