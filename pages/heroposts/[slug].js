...
const PostDetails = ({ heros, hero }) => {
  const { image, title, posturl, author, category, publishedAt, body } = hero;
...
export const getStaticPaths = async () => {
  ...
  const heros = await client.fetch(query);
...
export const getStaticProps = async ({ params: { slug } }) => {
  const query = `*[_type == "heros" && slug.current == '${slug}'][0]`;
  const herosQuery = '*[_type == "heros"]';
