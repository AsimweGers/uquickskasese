...
import { RTopStories } from "../../components";
import { PortableText } from "@portabletext/react";
import TimeAgo from "react-timeago";
import Head from "next/head";
import ReactPlayer from "react-player";

const PostDetails = ({ rwentopstories, rwentopstory }) => {
  ...
  return (
    <>
      <Head>
        <title>{title}</title>
        <meta name="keywords" content="Rwenzori Tv Top Stories" />
      ...
      <div className="items-center flex flex-col gap-2 p-2 ">
        ...
        <div className="flex flex-wrap gap-3">
          {rwentopstories.map((item) => (
            <RTopStories key={item._id} rwentopstory={item} />
          ))}
        </div>
      </div>
    </>
...
export const getStaticPaths = async () => {
  const query = `*[_type == "rwentopstories"] {
    slug {
      current
    }
  }
  ...
  const rwentopstories = await client.fetch(query);

  const paths = rwentopstories.map((rwentopstory) => ({
    params: {
      slug: rwentopstory.slug.current,
    },
  }));
...
export const getStaticProps = async ({ params: { slug } }) => {
  const query = `*[_type == "rwentopstories" && slug.current == '${slug}'][0]`;
  const rwentopstoriesQuery = '*[_type == "rwentopstories"]';

  const rwentopstory = await client.fetch(query);
  const rwentopstories = await client.fetch(rwentopstoriesQuery);

  return {
    props: { rwentopstories, rwentopstory },
  };
};
