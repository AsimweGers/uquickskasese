...
const PostDetails = ({ rgirls, rgirl }) => {
    ...
    <>
      ...
      <div className="items-center flex flex-col gap-2 p-2 ">
        ...
        <div className="flex flex-wrap gap-3">
          {rgirls.map((item) => (
            <GirlCard key={item._id} rgirl={item} />
          ))}
        </div>
      </div>
    </>
...
export const getStaticPaths = async () => {
  ...
  const paths = rgirls.map((rgirl) => ({
    params: {
      slug: rgirl.slug.current,
    },
  }));
...
export const getStaticProps = async ({ params: { slug } }) => {
  const query = `*[_type == "rgirls" && slug.current == '${slug}'][0]`;
  const rgirlsQuery = '*[_type == "rgirls"]';

  const rgirl = await client.fetch(query);
  const rgirls = await client.fetch(rgirlsQuery);
